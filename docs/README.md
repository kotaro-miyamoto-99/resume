...
- **担当業務詳細**
  - 担当生徒の管理者ページの開発
  生徒を管理する立場の人向けのページを開発。生徒毎の進捗が一目で分かりやすいようにグラフ化。
  - Django Viewsの開発
  Django Viewsを使用し生徒の登録・編集・削除を行うAPIを開発。
...
- **担当業務詳細**
  - LINE認証を組み込んだフロントエンド開発
  ユーザーの認証にLINE Front-end Framework (LIFF)を使用し、APIの実行時にアクセストークンを検証することでユーザーの認証を行う。
  - APIサーバのインフラ構築
  App RunnerとRDSを使用しAPIサーバの構築を行う。Github Actionsで自動デプロイを構築。
  - 大手小売会社の福利厚生導入プロジェクトの推進
  企業の福利厚生として導入してもらうためにセキュリティ部分を強化。また、担当者と打ち合わせを行い導入時の問題点や社員情報の識別方法などについてシステム要件化する。
  - 決済システムの導入（Fincode）
  医療薬品を扱える決済サービスの選定とサービス担当者との打ち合わせを行う。また、GMOが提供している決済サービスを使用しサブスクリプションの決済処理を実装。
  - Dockerを使用し環境構築の簡略化
  エンジニアが入った際に環境構築を容易に行えるようにDockerfileを構築しコマンドを実行するだけで開発に入れる体制を構築。また、シードデータを入れるコマンドを構築し環境構築に取られる時間を削減
  - Notionでのエンジニアのタスク管理
  プロダクトバックログの優先度付けを行い、開発メンバー（他2人）へ割り振るタスクを管理。